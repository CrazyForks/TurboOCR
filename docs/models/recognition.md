...
1. **Crop widths vary wildly.** A page mixes lines of 80 px and 1600 px, and
   ...
   ([`paddle_rec.h:58`](https://github.com/aiptimizer/TurboOCR/blob/main/include/turbo_ocr/recognition/paddle_rec.h)) and
...
2. **Per-batch sync kills GPU utilisation.** A naïve loop is `H2D → execute →
   ...
   **multi-slot deferred-sync** loop
   ([`paddle_rec.cpp:122-244`](https://github.com/aiptimizer/TurboOCR/blob/main/src/recognition/paddle_rec.cpp)): up to
   `kMaxSlots = 20` ([`paddle_rec.h:76`](https://github.com/aiptimizer/TurboOCR/blob/main/include/turbo_ocr/recognition/paddle_rec.h))
   ...
   [`paddle_rec.cpp:122-130`](https://github.com/aiptimizer/TurboOCR/blob/main/src/recognition/paddle_rec.cpp) calls out
...
([`paddle_rec.h:80-84`](https://github.com/aiptimizer/TurboOCR/blob/main/include/turbo_ocr/recognition/paddle_rec.h)).
...
| Precision | FP16 |
| Batch | `rec_batch_num_ = 32` ([`paddle_rec.h:51`](https://github.com/aiptimizer/TurboOCR/blob/main/include/turbo_ocr/recognition/paddle_rec.h)), configurable via `set_batch_num` |
...
[`paddle_rec.h:91-95`](https://github.com/aiptimizer/TurboOCR/blob/main/include/turbo_ocr/recognition/paddle_rec.h))
...
mutex is taken at the top of `run()` ([`paddle_rec.cpp:92`](https://github.com/aiptimizer/TurboOCR/blob/main/src/recognition/paddle_rec.cpp)).
...
single-engine path ([`ocr_pipeline.cpp:521-538`](https://github.com/aiptimizer/TurboOCR/blob/main/src/pipeline/ocr_pipeline.cpp)
and the surrounding `dispatch_rec_`).

`run_multi(...)` ([`paddle_rec.cpp:246-432`](https://github.com/aiptimizer/TurboOCR/blob/main/src/recognition/paddle_rec.cpp))
...
`ctc_greedy_decode` lives in `src/recognition/ctc_decode.h` and runs purely on
...
([`ocr_pipeline.cpp:517-…`](https://github.com/aiptimizer/TurboOCR/blob/main/src/pipeline/ocr_pipeline.cpp)) on
`rec_stream_`. Recognition's only upstream wait is `det_event_`, recorded after
det+cls finish on the caller stream
([`ocr_pipeline.cpp:684-687`](https://github.com/aiptimizer/TurboOCR/blob/main/src/pipeline/ocr_pipeline.cpp)). When rec
