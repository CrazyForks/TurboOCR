...
Re-run (after building the server):
  python3 scripts/bench_speed_matrix.py                       # full matrix, fresh baseline
  python3 scripts/bench_speed_matrix.py --baseline result/bench_speed_matrix.json   # regression check
  python3 scripts/bench_speed_matrix.py --only both_vl text_only --n 40 --concurrency 8
...
def load_images(a):
    ...
    scripts/omnidoc_subset_n.py into /tmp/omnidoc_subset<N> — so speed and accuracy run on
    ...
    if a.images_set == "subset":
        ...
        gen = REPO / "scripts" / "omnidoc_subset_n.py"
        ...
        if subset_gt.is_file():
            ...
            prov = (f"OmniDocBench {a.n}-doc stratified subset via scripts/omnidoc_subset_n.py "
...
def main() -> int:
    ...
    p.add_argument("--images-set", choices=["subset", "dir"], default="subset",
                   help="subset = the SAME stratified OmniDocBench subset as the accuracy harness "
                        "(scripts/omnidoc_subset_n.py, default); dir = first-N from --images-dir")
