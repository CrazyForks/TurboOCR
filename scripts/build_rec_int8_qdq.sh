...
#   3. Run scripts/quantize_onnx_int8.py --model-type rec → models/rec_qdq.onnx
...
#      tools/omnidoc_run_and_score.py + a prior baseline run. If not present
...
VENV_PY="${VENV_PY:-/home/nataell/code/epAiland/paddle-highspeed-cpp/.venv-modelopt/bin/python}"
...
if [[ -f "${REC_QDQ}" ]] && [[ "${REC_QDQ}" -nt "${REC_ONNX}" ]]; then
  ...
  "${VENV_PY}" "${REPO_ROOT}/scripts/quantize_onnx_int8.py" \
...
if ! "${VENV_PY}" "${REPO_ROOT}/scripts/int8_latency_gate.py" \
...
if [[ ! -f "${BASELINE_SUMMARY}" ]]; then
  ...
  if ! timeout 1200s "${VENV_PY}" "${REPO_ROOT}/scripts/omnidoc_run_and_score.py" \
  ...
  if ! "${VENV_PY}" "${REPO_ROOT}/scripts/omnidoc_diff.py" \
