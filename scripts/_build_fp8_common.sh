...
#
# Mirrors scripts/_build_int8_common.sh almost verbatim — different env-var
...
VENV_PY="${VENV_PY:-/home/nataell/code/epAiland/.venv-paddle/bin/python}"
...
run_fp8_build() {
  ...
  if [[ -f "${qdq_path}" ]] && [[ "${qdq_path}" -nt "${ONNX_PATH}" ]]; then
    ...
    "${VENV_PY}" "${REPO_ROOT}/scripts/quantize_onnx_fp8.py" \
  ...
  if ! "${VENV_PY}" "${REPO_ROOT}/scripts/fp8_latency_gate.py" \
