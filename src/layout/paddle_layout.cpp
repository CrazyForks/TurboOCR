...
PaddleLayout::collect_batch(float score_threshold) {
  ...
  // Per-image detection counts come from the model's own count tensor (out1),
  // NOT from getTensorShape(out0): out0's first dim is data-dependent and
  // TensorRT reports it unreliably across repeated executions (same root cause
  // fixed in collect()). The model writes each image's rows contiguously, so
  // the total row count is the sum of the per-image counts.
  ...
  for (int i = 0; i < pending_batch_; ++i) {
    int n = h_out1_.get()[i];
    if (n < 0) n = 0;
    total_rows += std::min(n, kMaxDetections);
  }
  const int rows_cap = std::min(total_rows, pending_batch_ * kMaxDetections);

  // D2H the packed bbox tensor for exactly the rows the counts say exist.
  if (rows_cap > 0) {
