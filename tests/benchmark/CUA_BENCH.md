...
Entry point: `scripts/bench_cua_loop.sh` (called every 10 min).
...
- `tests/benchmark/bench_cua_router.py` — orchestrator (CLI entry)
- `tests/benchmark/_cua_scenarios.py` — fixtures, thresholds, self-validator
- `tests/benchmark/_regression.py` — rolling baseline + verdict
- `scripts/bench_cua_loop.sh` — health probe + python invocation + exit map
