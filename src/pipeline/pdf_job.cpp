// Implementation of the transport-agnostic PDF orchestrator declared in
// pdf_job.h. Split out of the header so the 800-line body is compiled once per
// build config (GPU / CPU) instead of inlined into every includer. Only the
// public surface — the PdfJob* structs, serialize_page_results(), and
// run_pdf_job() — lives in the header; everything here has internal linkage.
//
// Compiled into turboocr-server (GPU: both run_pdf_job overloads) and
// turboocr-cpu-server (USE_CPU_ONLY: the InferFunc overload + shared helpers).
