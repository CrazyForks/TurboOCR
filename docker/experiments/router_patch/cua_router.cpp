...
[[nodiscard]] inline bool aabb_centroid_in(
...
// Returns min(width, height) of an AABB — used for min_crop_px gate.
[[nodiscard]] inline int aabb_min_side(const std::array<int, 4> &r) noexcept {
  return std::min(std::max(0, r[2] - r[0]), std::max(0, r[3] - r[1]));
}
...
void build_overlap_stats(
  ...
  for (const auto &b : det_boxes) {
    ...
    // Centroid match — same rule used by assign_layout_ids() so the
    // router and the serializer agree on which cell owns a det box.
    float cx = 0.0f, cy = 0.0f;
    for (int k = 0; k < 4; ++k) {
      cx += static_cast<float>(b[k][0]);
      cy += static_cast<float>(b[k][1]);
    ...
    for (std::size_t i = 0; i < n; ++i) {
      ...
      if (cx >= static_cast<float>(r[0]) && cx <= static_cast<float>(r[2]) &&
          cy >= static_cast<float>(r[1]) && cy <= static_cast<float>(r[3])) {
        out.det_count[i] += 1;
...
CuaRouter::CuaRouter() noexcept {
  ...
  // Apply env-tunable thresholds at construction time.
  cfg_ = RouterConfig::from_env();
}
...
void CuaRouter::classify(const std::vector<turbo_ocr::Box> &det_boxes,
  ...
  //    ROUTER_MIN_CROP_PX: Formula/Table boxes whose min(w,h) < threshold
  //    are demoted to Text (their det boxes fall through to rec path).
  ...
  for (auto &d : decisions_) {
    if (cfg_.min_crop_px > 0 &&
        (d.dest == Destination::Formula || d.dest == Destination::Table)) {
      const auto &r = layout_aabbs_[static_cast<std::size_t>(d.layout_idx)];
      if (aabb_min_side(r) < cfg_.min_crop_px) {
        d.dest = Destination::Text;
        d.wrap = FormulaWrap::None;
      }
    }
  ...
  for (std::size_t i = 0; i < det_boxes.size(); ++i) {
    ...
    for (std::size_t li = 0; li < layout_aabbs_.size(); ++li) {
      if (aabb_centroid_in(layout_aabbs_[li], det_boxes[i])) {
  ...
  {
    int n_text = 0, n_table = 0, n_formula = 0, n_skip = 0;
    for (const auto &d : decisions_) {
      switch (d.dest) {
        case Destination::Text:    ++n_text; break;
        case Destination::Table:   ++n_table; break;
        case Destination::Formula: ++n_formula; break;
        case Destination::Skip:    ++n_skip; break;
      }
    }
    TOCR_LOG_DEBUG("[router] classify",
                  "det", static_cast<int>(det_boxes.size()),
                  "layout", static_cast<int>(layout.size()),
                  "text", n_text,
                  "table", n_table,
                  "formula", n_formula,
                  "skip", n_skip,
                  "page_has_confident_formula",
                  page_stats_.has_confident_formula ? 1 : 0);
  }
}

void CuaRouter::dispatch_tables(
    const std::vector<turbo_ocr::layout::LayoutBox> & /*layout*/,
    const RoutingPlan & /*plan*/) const noexcept {
  // Wired up in T18 / T19 once TablePipeline lands.
  (void)table_;
}

void CuaRouter::dispatch_formulas(
    const std::vector<turbo_ocr::layout::LayoutBox> & /*layout*/,
    const RoutingPlan & /*plan*/) const noexcept {
  (void)formula_;
}
