...
#
# Usage:
#   scripts/exp_runner.sh <experiment_name> [docker -e KEY=VAL ...]
#
# Example:
#   scripts/exp_runner.sh table_fp16_pure -e TURBO_OCR_TABLE_FP16=1
...
if ! timeout "${SCORE_TIMEOUT}" python3 "${REPO_ROOT}/scripts/omnidoc_run_and_score.py" \
...
DIFF_EXIT=0
python3 "${REPO_ROOT}/scripts/omnidoc_diff.py" "${BASELINE}" "${RESULT_JSON}" --threshold 0.01 || DIFF_EXIT=$?
