...
var is set (see src/engine/onnx_to_trt.cpp `int8_onnx_path_for`).

Calibration source: scripts/calibration_data.py picks 200 stratified
...
def _preprocess_det(path: Path, max_side: int = 960) -> np.ndarray:
    # Matches src/detection/paddle_det.cpp + the fused_resize_normalize_chw kernel:
...
def _preprocess_layout(path: Path, size: int = 800) -> dict:
    ...
    # Profile lives in src/engine/onnx_to_trt.cpp:266-292.
    img = cv2.imread(str(path))
    if img is None:
        raise RuntimeError(f"cannot read {path}")
...
def _preprocess_rec_crop(crop_bgr: np.ndarray, target_h: int = 48) -> np.ndarray:
    # Matches src/recognition/paddle_rec.cpp + the fused batch_roi_warp kernel:
...
def _preprocess_formula_encoder(path: Path, h: int = 192, w: int = 672) -> np.ndarray:
    ...
    # (the v1 single-profile pin in src/engine/onnx_to_trt.cpp:381-390). Real
...
def run_quantize(onnx_path: Path, model_type: str, out_path: Path,
    ...
    try:
        ...
        print("  fix: run with /home/nataell/code/epAiland/paddle-highspeed-cpp/"
